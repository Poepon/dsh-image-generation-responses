...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register(defineTool({
    name: "analyze_image",
    description:
      "Answer a question about images from this conversation — describe content, read text, compare, or inspect details. The images are read by a vision model over the same Responses endpoint, so the conversation model itself does not need image input. Pass attachment ids of images already present in this conversation (for example ones returned by generate_image).",
    parameters: {
      question: {
        type: "string",
        required: true,
