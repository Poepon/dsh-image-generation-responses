...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var MessageImage = uiAttachment.MessageImage;
    ...
    var CSS = [
      ...
      // Overlay panel: a full-height vertical dock pinned to the conversation
      // column's left edge. The shell overlay layer is click-through and spans
      // the frame, so the dock opts back into pointer events and positions
      // itself against the measured sidebar width.
      ".dsh-igr-panel{position:absolute;top:0;bottom:0;z-index:30;display:flex;flex-direction:column;",
      "width:264px;max-width:60vw;pointer-events:auto;",
      "border-right:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);",
      "box-shadow:6px 0 18px rgba(0,0,0,.14);overflow:hidden}",
      ...
      ".dsh-igr-panel-body{flex:auto;min-height:0;overflow-y:auto;overflow-x:hidden;padding:12px}",
      // The vertical stack: one image per row, newest at the top.
      ".dsh-igr-stack{display:flex;flex-direction:column;align-items:flex-start;gap:12px}",
      ".dsh-igr-stack>*{flex:none;max-width:100%}",
    ].join("");
     ...
     *
     * The shell overlay layer is a grid child spanning the whole frame, so an
     * absolutely positioned occupant is placed relative to the frame — meaning
     * the sidebar column's rendered width IS the dock's `left`. That width is
     * owner state of the `sidebar` slot, which this root-scoped seat cannot
     * read, and it changes on collapse and on drag. So it is measured from the
     * live column box, with a ResizeObserver when the platform provides one and
     * a window-resize fallback otherwise.
    ...
    /**
     * The `shell.overlay` entry: a full-height vertical dock against the
     * conversation column's left edge. Renders `null` while closed, so the
     * click-through overlay layer stays empty until asked for.
     */
    function makeGalleryPanel(store, loaderFor, sessionsOf) {
      return function SessionImagesPanel(props) {
        ...
        var offset = useSidebarOffset(open);
        ...
        if (sessionId === undefined) {
          ...
          // One image per row. ImageGallery is a wrapping flex group that
          // switches to 64px tiles past a single image, so the vertical stack
          // drives MessageImage directly and keeps every entry at `single` size.
          var load = loaderFor(sessionId);
          body = h("div", { className: "dsh-igr-stack" }, images.map(function (item) {
            return h(MessageImage, {
