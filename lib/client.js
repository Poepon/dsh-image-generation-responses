 ...
 * 2. A session image dock: a vertically centred strip of every durable image in
 *    the current session (generated, uploaded, or tool-produced), newest first.
 *    It has no toggle — it appears exactly when the conversation holds an image
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * `tool.call.toolview` slot, and add the session image dock as an additive
     * `shell.overlay` entry. Every registration is wrapped in
     * `ctx.slots.inject` and therefore unwinds with the fiber.
